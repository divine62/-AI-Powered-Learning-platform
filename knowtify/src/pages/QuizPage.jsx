...
const GEMINI_KEY = "AIzaSyCdfOm9qY6GHdisZJHOE3p-SeMg-d5nxeA";
...
export default function QuizPage() {
  ...
  const generateQuiz = async () => {
    ...
    const prompt = `Generate exactly 10 multiple choice questions for Indian engineering students on the subject: "${finalSubject}".
...
[
  ...
  // ── Render phases ─────────────────────────────────────────────────────────
  if (phase === "setup")    return <SetupScreen subject={subject} setSubject={setSubject} customSub={customSub} setCustomSub={setCustomSub} difficulty={difficulty} setDiff={setDiff} error={error} onStart={generateQuiz} />;
...
// ─── Setup Screen ─────────────────────────────────────────────────────────────
function SetupScreen({ subject, setSubject, customSub, setCustomSub, difficulty, setDiff, error, onStart }) {
  return (
            ...
            <div style={{ display: "flex", gap: "8px" }}>
              {["Easy", "Medium", "Hard"].map(d => (
                <button key={d} onClick={() => setDiff(d)} style={{
                  ...chipBtn(difficulty === d),
                  flex: 1,
                  padding: "10px",
                  color: difficulty === d
                    ? d === "Easy" ? "#4ade80" : d === "Hard" ? "#f87171" : "var(--nude)"
                    : "rgba(244,239,233,0.35)",
                  borderColor: difficulty === d
                    ? d === "Easy" ? "#4ade80" : d === "Hard" ? "#f87171" : "var(--nude)"
                    : "var(--card-border)",
                  background: difficulty === d
                    ? d === "Easy" ? "rgba(74,222,128,0.1)" : d === "Hard" ? "rgba(248,113,113,0.1)" : "rgba(200,168,130,0.1)"
                    : "transparent",
                }}>{d}</button>
              ))}
            </div>
...
function LoadingScreen({ subject }) {
  return (
    <div style={{ minHeight: "100vh", background: "var(--black)", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: "20px" }}>
      ...
      <p style={{ fontFamily: "DM Mono, monospace", fontSize: "11px", color: "#555" }}>
        AI is crafting 10 questions for you
      </p>
...
function QuizScreen({ questions, current, selected, revealed, answers, onSelect, onNext }) {
  ...
  const optionStyle = (letter) => {
    ...
    if (letter === correct) return { ...base, background: "rgba(74,222,128,0.12)", border: "1px solid rgba(74,222,128,0.4)", color: "#4ade80" };
    if (letter === selected && letter !== correct) return { ...base, background: "rgba(248,113,113,0.1)", border: "1px solid rgba(248,113,113,0.3)", color: "#f87171" };
    ...
    <div style={{ paddingTop: "100px", paddingBottom: "80px", minHeight: "100vh", background: "var(--black)" }}>
      <div style={{ maxWidth: "640px", margin: "0 auto", padding: "0 24px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "12px" }}>
          <span style={{ fontFamily: "DM Mono, monospace", fontSize: "10px", color: "#555", letterSpacing: "0.12em" }}>
            QUESTION {current + 1} OF {total}
          </span>
          <span style={{ fontFamily: "DM Mono, monospace", fontSize: "10px", color: "var(--nude)" }}>
            {done} answered
          </span>
        </div>
        ...
        <div className="glass-card" style={{ padding: "32px", marginBottom: "20px" }}>
          ...
          {q.options.map((opt) => {
            ...
            return (
              <button key={letter} onClick={() => onSelect(letter)} style={optionStyle(letter)}>
                {opt}
              </button>
            );
          })}
          ...
          {revealed && (
            <div style={{ marginTop: "16px", padding: "14px 16px", background: "rgba(200,168,130,0.06)", border: "1px solid rgba(200,168,130,0.15)", borderRadius: "8px" }}>
              ...
              <p style={{ fontFamily: "DM Mono, monospace", fontSize: "11px", color: "rgba(244,239,233,0.6)", marginTop: "6px", lineHeight: 1.6 }}>
                {q.explanation}
              </p>
            </div>
          )}
    ...
    <div style={{ paddingTop: "100px", paddingBottom: "80px", minHeight: "100vh", background: "var(--black)" }}>
      <div style={{ maxWidth: "640px", margin: "0 auto", padding: "0 24px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "center", margin: "32px 0" }}>
          <svg width="140" height="140" viewBox="0 0 140 140">
            ...
            <text x="70" y="65" textAnchor="middle" dominantBaseline="central"
              style={{ fontSize: 28, fontFamily: "Cormorant Garamond, serif", fill: color, fontWeight: 500 }}>
              {pct}%
            </text>
            <text x="70" y="88" textAnchor="middle"
              style={{ fontSize: 9, fontFamily: "DM Mono, monospace", fill: "#555", letterSpacing: "0.1em" }}>
        ...
        <div className="glass-card" style={{ padding: "24px", marginBottom: "20px" }}>
          <p style={{ fontFamily: "DM Mono, monospace", fontSize: "10px", letterSpacing: "0.15em", textTransform: "uppercase", color: "#555", marginBottom: "16px" }}>
            Question Breakdown
          </p>
              ...
              return (
                <div key={i} style={{ display: "flex", gap: "12px", alignItems: "flex-start" }}>
                  <div style={{
                    ...
                    border: `1px solid ${isCorrect ? "rgba(74,222,128,0.4)" : "rgba(248,113,113,0.3)"}`,
                    display: "flex", alignItems: "center", justifyContent: "center",
                    fontSize: "10px", color: isCorrect ? "#4ade80" : "#f87171",
                  }}>
                    {isCorrect ? "✓" : "✗"}
                  </div>
                  <div style={{ flex: 1 }}>
                    <p style={{ fontFamily: "DM Mono, monospace", fontSize: "11px", color: "rgba(244,239,233,0.7)", marginBottom: "4px", lineHeight: 1.4 }}>
                      {q.question}
                    </p>
                    {!isCorrect && (
        ...
        <div style={{ display: "flex", gap: "12px" }}>
          <button onClick={onRetry} style={{ ...primaryBtn, flex: 1, background: "transparent", color: "var(--nude)", border: "1px solid rgba(200,168,130,0.3)" }}>
            Try Again
          </button>
          <button onClick={onDashboard} style={{ ...primaryBtn, flex: 2 }}>
            Back to Dashboard →
          </button>
        </div>
      </div>
